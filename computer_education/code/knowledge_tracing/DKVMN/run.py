...
def train(epoch_num, model, params, optimizer, q_data, qa_data, binary):
    ...
    for idx in range(N):
        ...
        qa_target = qa_data[idx * params.batch_size:(idx + 1) * params.batch_size, :]

        # target = (target - 1) / params.n_question
        # target = np.floor(target)

        # 恢复到原来的结果，通过mask屏蔽掉后面用0填充的序列
        mask = qa_target > 0
        target = -1 * np.ones(qa_target.shape)
        if binary:
            target[mask] = qa_target[mask] % 2
        else:
            target[mask] = (qa_target[mask] + 1) % 12
...
def test(model, params, optimizer, q_data, qa_data, binary):
    N = int(math.floor(len(q_data) / params.batch_size))
