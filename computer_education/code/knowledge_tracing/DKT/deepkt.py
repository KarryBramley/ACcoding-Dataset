...
class DKTModel(tf.keras.Model):
    """ The Deep Knowledge Tracing models.
    ...
    Raises:
        ValueError: In case of mismatch between the provided input data
            and what the models expects.
    """

    def __init__(self, nb_features, nb_skills, hidden_units=100, dropout_rate=0.2):
        inputs = tf.keras.Input(shape=(None, nb_features), name='inputs')

        x = tf.keras.layers.Masking(mask_value=data_util.MASK_VALUE)(inputs)    #屏蔽值是MASK_VALUE的项
    ...
    def compile(self, optimizer, metrics=None):
        ...
        Arguments:
            ...
            metrics: List of metrics to be evaluated by the models during training
                and testing. Typically you will use `metrics=['accuracy']`.
                To specify different metrics for different outputs of a
                multi-output models, you could also pass a dictionary, such as
    ...
    def fit(self,
            ...
            validation_freq=1):
        """Trains the models for a fixed number of epochs (iterations on a dataset).
        Arguments:
            dataset: A `tf.data` dataset. Should return a tuple
                of `(inputs, (skills, targets))`.
            epochs: Integer. Number of epochs to train the models.
                An epoch is an iteration over the entire data provided.
                Note that in conjunction with `initial_epoch`,
                `epochs` is to be understood as "final epoch".
                The models is not trained for a number of iterations
            ...
            validation_data: Data on which to evaluate
                the loss and any models metrics at the end of each epoch.
                The models will not be trained on this data.
        ...
        Raises:
            RuntimeError: If the models was never compiled.
            ValueError: In case of mismatch between the provided input data
                and what the models expects.
        """
    ...
    def evaluate(self,
        ...
        Returns:
            Scalar test loss (if the models has a single output and no metrics)
            or list of scalars (if the models has multiple outputs
            and/or metrics). The attribute `models.metrics_names` will give you
    ...
    def predict(self,
                ...
                steps=None,
                callbacks=None):
        return super(DKTModel, self).predict(dataset,
                                             verbose=verbose,
                                             steps=steps,
                                             callbacks=callbacks)
